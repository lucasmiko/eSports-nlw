...
export function CreateAdModal() {
    ...
    async function handleCreateAd(eventOfClick: FormEvent ){
        eventOfClick.preventDefault();
        
        const formData = new FormData(eventOfClick.target as HTMLFormElement)
        const data = Object.fromEntries(formData)

        try {
                axios.post(`http://localhost:3333/games/${data.game}/ads`, {
                name: data.name,
                yearsPlaying: String(data.yearsPlaying),
